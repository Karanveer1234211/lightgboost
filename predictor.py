# UPDATED v5.9 — train-only panel + separate inference tail for fresh watchlist
# predictor_v5_9_infer_tail.py
#
# This version keeps your panel_cache.parquet strictly "train-only" (rows must have labels),
# and collects the latest engineered rows from the daily cache separately for inference.
# Watchlist predictions use these freshest rows when available; training, SHAP, OOS, WhatWorked
# continue to use only labeled panel rows (same behavior as your original v5.9).
#
...
COMPUTE_SHAP_FOR = ["1d","3d"]
SHAP_MAX_SYMBOLS = None
...
HAC_LAG = "auto"
THIN_INFERENCE = True
# Prob std options for 5D
PROB_STD_METHOD = "residual"
PROB_STD_WINDOW = 252
PROB_STD_MIN_ROWS = 60
# 1D classification margin
CLS_MARGIN = 0.08
...
def parse_cli():
    ...
    ap.add_argument("--embargo-days", type=int, default=EMBARGO_DAYS,
        help="Embargo gap in days between train/test split (default=3).")
    ...
    ap.add_argument("--max-combo-size", type=int, default=MAX_COMBO_SIZE,
        help="Maximum combo size for WhatWorked (default=5).")
...
def featureize(df: pd.DataFrame):
    ...
    if yday_unified.notna().any():
        df = df.drop(columns=[c for c in df.columns
                              if c == "D_cpr_vs_yday" or c.startswith("D_cpr_vs_yday__dup")],
                     errors="ignore")
    ...
    if tmr_unified.notna().any():
        df = df.drop(columns=[c for c in df.columns
                              if c == "D_tmr_cpr_vs_today" or c.startswith("D_tmr_cpr_vs_today__dup")],
                     errors="ignore")
...
def _prepare_panel_rows(path_obj: Path, min_ts_map: dict):
    ...
    try:
        ...
        df = score_bias(df)
        df["symbol"] = sym
        # TRAIN-ONLY subset (requires labels)
...
# ===================== INFERENCE LATEST (unlabeled) =====================
def _prepare_inference_rows(path_obj: Path, min_ts_map: dict):
    """Return newest engineered rows for a symbol for inference only (even if labels are NaN)."""
    sym = _derive_symbol_name(path_obj)
    try:
        df = load_one(path_obj)
        df["timestamp"] = ensure_kolkata_tz(df["timestamp"])
        ...
        df = score_bias(df)
        df["symbol"] = sym
        keep_cols = ["timestamp","symbol","open","high","low","close","volume"] + feats + [
            "ret_1d_close_pct","ret_3d_close_pct","ret_5d_close_pct",
            "long_score","short_score","D_atr14","D_cpr_width_pct"
        ]
...
def collect_inference_latest(paths: List[Path], min_ts_map: dict, load_workers: int = 1) -> pd.DataFrame:
    """Collect one latest unlabeled day per symbol for inference (do NOT write into panel)."""
    expanded: List[Path] = []
    for p in paths:
        expanded += _strict_file_list(str(p), None, None, accept_any_daily=False) if Path(p).is_dir() else [Path(p)]
    paths = sorted(expanded)
    if len(paths) == 0:
        return pd.DataFrame()

    eta = ProgressETA(total=len(paths), label="InferLatest")
    parts: List[pd.DataFrame] = []

    def _go(path_obj: Path): return path_obj, _prepare_inference_rows(path_obj, min_ts_map)

    try:
        if load_workers > 1:
            with concurrent.futures.ThreadPoolExecutor(max_workers=int(load_workers)) as ex:
                for path_obj, result in ex.map(_go, paths):
                    sym, rows, msg_or_err = result
                    if isinstance(msg_or_err, Exception):
                        _log_load_error(sym, str(path_obj), str(msg_or_err))
                        eta.tick(f"ERR {sym}: {msg_or_err}")
                        continue
                    if rows is None:
                        eta.tick(msg_or_err or f"NO INFER ROWS {sym}")
                        continue
                    parts.append(rows)
                    eta.tick(f"OK {sym} (+{len(rows)} infer)")
        else:
            for path_obj in paths:
                sym, rows, msg_or_err = _prepare_inference_rows(path_obj, min_ts_map)
                if isinstance(msg_or_err, Exception):
                    _log_load_error(sym, str(path_obj), str(msg_or_err))
                    eta.tick(f"ERR {sym}: {msg_or_err}")
                    continue
        ...
        print("\nInterrupted (inference collector).")
        raise
...
def sanitize_feature_matrix(X: pd.DataFrame) -> pd.DataFrame:
    ...
    # --- NEW: drop duplicated columns to keep only the first occurrence ---
    # This avoids pandas returning a DataFrame when you select X[c] by a label that exists multiple times.
    ...
    for c in X.columns:
        ...
        # If, for any reason, X[c] is a DataFrame (duplicate label), collapse to the first column
        # (This is a second guard; the de-dup above should already fix it)
        if isinstance(ser, pd.DataFrame):
            ser = ser.iloc[:, 0]
            X[c] = ser  # overwrite with the first occurrence

        # Now 'ser' is a Series → safe to use .dtype
        if ser.dtype == bool:
            X[c] = ser.astype(int)
        ...
        # Ensure all one-hot columns are int 0/1
        if c.startswith(("CPR_Yday_", "CPR_Tmr_", "Struct_", "DayType_")):
...
# MODIFIED: accept latest_override for freshest inference rows
def nightly_watchlist(panel: pd.DataFrame, feats: List[str],
                      ...
                      latest_override: Optional[pd.DataFrame] = None):
...
def compute_shap_latest(panel: pd.DataFrame, feats: List[str], model, top_k: int = TOP_SHAP_PER_SYMBOL, shap_max_symbols: Optional[int] = None):
    ...
    for i, (_, row) in enumerate(latest.iterrows()):
        ...
        eta.tick(sym)
    shap_df = pd.DataFrame(rows).sort_values(["symbol","rank"]) if rows else pd.DataFrame()
    g_rows = [{"feature": f, "mean_abs_shap": float(np.mean(v))} for f, v in global_abs.items()]
    global_df = pd.DataFrame(g_rows).sort_values("mean_abs_shap", ascending=False) if g_rows else pd.DataFrame()
...
# ---- Meta helpers (last train date per horizon) ----
def load_meta() -> dict:
    p = Path(META_PATH)
    if not p.exists():
        return {}
...
def _recent_accuracy_for_horizon(panel: pd.DataFrame, oos_path: Path, horizon: str, last_days: int) -> dict:
    ...
    # CLASSIFIER PATH for 1D
    is_prob_pred = (horizon == "1d") and (np.nanmin(pred) >= 0.0) and (np.nanmax(pred) <= 1.0)
    if is_prob_pred:
        from sklearn.metrics import log_loss as _sk_log_loss, brier_score_loss, roc_auc_score, roc_curve
        r1d = pd.to_numeric(panel.loc[idx, "ret_1d_close_pct"], errors="coerce").values
        y = np.full_like(r1d, np.nan, dtype=float)
        y[r1d > CLS_MARGIN] = 1.0
        y[r1d < -CLS_MARGIN] = 0.0
        mask = ~np.isnan(y)
        y = y[mask].astype(int)
        ...
        if rows == 0:
            return {"horizon": horizon, "window_days": last_days, "rows": 0,
                    "hit_rate_%": np.nan, "mae_%": np.nan, "rmse_%": np.nan,
                    "mean_pred_%": np.nan, "mean_real_%": np.nan, "pearson_r": np.nan,
                    "wilson_low": np.nan, "wilson_high": np.nan, "logloss": np.nan,
                    "brier": np.nan, "auc": np.nan, "threshold_used": np.nan}
        ll = _sk_log_loss(y, p)
        ...
        try:
            ...
            j = tpr - fpr
            thr_use = float(thr[np.argmax(j)]) if len(thr) else 0.5
        except Exception:
            thr_use = 0.5
        y_hat = (p >= thr_use).astype(int)
        hits = (y_hat == y).sum()
        total = int(len(y_hat))
        hit_rate = 100.0 * hits / total if total > 0 else np.nan
        wl, wh = wilson_ci(hits, total, 1.96) if total > 0 else (np.nan, np.nan)
        return {"horizon": horizon, "window_days": last_days, "rows": rows,
                "hit_rate_%": hit_rate, "mae_%": np.nan, "rmse_%": np.nan,
                "mean_pred_%": np.nan, "mean_real_%": np.nan, "pearson_r": np.nan,
                "wilson_low": wl, "wilson_high": wh,
                "logloss": ll, "brier": br, "auc": auc, "threshold_used": thr_use}
    # REGRESSION PATH
...
def _symbol_accuracy_oos(panel: pd.DataFrame, oos_df: pd.DataFrame, horizon: str, thin_inference: bool = True) -> pd.DataFrame:
    ...
    df = pd.DataFrame(rows).sort_values(["symbol","horizon"]) if rows else pd.DataFrame()
    return df
...
def main():
    ...
    # 0) Panel load/collect (train-only panel)
    ...
    # 0.1) Build inference-latest (unlabeled) if cache is ahead of panel
    panel_last_map = last_ts_by_symbol_from_panel(PANEL_OUT)
    paths_for_infer = resolve_input_paths(args)
    infer_latest = collect_inference_latest(paths_for_infer, panel_last_map, load_workers=int(args.load_workers))
    ...
    wl = nightly_watchlist(panel, feats,
                           ...
                           latest_override=(infer_latest if infer_latest is not None and not infer_latest.empty else None))
    ...
    try:
        ...
        print(f"WARNING: WhatWorked failed ({e})")
